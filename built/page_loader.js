...
export function initStationsPanel() {
    ...
    function _stationCard(id, names, num) {
        return $('<div>', { 'id': id }).addClass('mdc-card mdc-layout-grid__cell--span-2-desktop mdc-layout-grid__cell--span-4-tablet mdc-layout-grid__cell--span-2-phone station-card').append($('<div>').addClass('mdc-card__primary-action').append($('<div>').addClass('mdc-card__media mdc-card__media--16-9')).append($('<div>').addClass('mdc-card__media-content station-card__content')
            .html(names.join('<br>'))
            .prepend($('<span>', { style: (window.urlParams.get('style') == 'gzmtr' ? '' : 'display:none;') }).text(num + ' ')))).append($('<div>').addClass('mdc-card__actions').append($('<div>').addClass('mdc-card__action-icons').append($('<button>', { title: 'Set As Current' }).addClass('material-icons mdc-icon-button mdc-card__action mdc-card__action--icon').text('my_location')).append($('<button>', { title: 'Interchange' }).addClass('material-icons mdc-icon-button mdc-card__action mdc-card__action--icon').text('edit')).append($('<button>', { title: 'Remove' }).addClass('material-icons mdc-icon-button mdc-card__action mdc-card__action--icon').text('delete_forever'))));
    }
    ...
    const initBranch = (stnInfo) => {
        ...
        Promise.resolve(getParams().stn_list)
            ...
            stnInfo.parents.forEach(par => {
                $('#left_first__selection').append($('<li>', {
                    class: 'mdc-list-item',
                    'data-value': par
                }).text(stnList[par].name.join(' - ')));
            });
            stnInfo.children.forEach(child => {
                $('#right_first__selection').append($('<li>', {
                    class: 'mdc-list-item',
                    'data-value': child
    ...
    $('#stn_transfer_diag')[0].MDCDialog.listen('MDCDialog:closed', event => {
        ...
        var [intInfo0, intInfo1, intInfo2] = [0, 1, 2].map(idx => {
            return $('#int_city, #int_line')
                ...
                .attr('style').match(/#[\w\d]+/g), $('#int_name_zh, #int_name_en').find('.mdc-text-field').slice(idx * 2, (idx + 1) * 2).get().map(el => el.MDCTextField.value));
        });
        if (type == 'none') {
    ...
    $('#left_first, #right_first').each((_, el) => {
        el.MDCSelect.listen('MDCSelect:change', event => {
            ...
            if ($(`#${direc}_first__selection`).children().length === 1) {
                return;
            }
            let stnId = $('#stn_transfer_diag').attr('for');
