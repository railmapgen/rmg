...
class Line {
  constructor(param) {
    ...
    _defineProperty(this, "_lineNames", void 0);
  ...
  get lineXs() {
    return [this._svgWidth * this._padding / 100, this._svgWidth * (1 - this._padding / 100)];
  }
  ...
  drawDestInfo() {
    ...
    $('#dest_name > g:last-child text').eq(1).text(`${lineNameEN}to ${destNameEN}`); // $('#dest_name > g:last-child').empty().attr('text-anchor', txtAnchor).append(
...
class LineGZ extends Line {
  constructor(param) {
    ...
    _classPrivateFieldSet(this, _infoPanelType, param.info_panel_type);
  }

  _initStnInstance(stnId, stnInfo) {
    switch (stnInfo.change_type) {
      case 'int2': // return new Int2StationGZ(stnId, stnInfo);
      ...
      case 'osi11_pl':
      case 'osi11_ur':
      case 'osi11_pr': // return new OSI11StationGZ(stnId, stnInfo);
  ...
  get lineXs() {
    if (this._direction == 'r') {
      return [this._svgWidth * this._padding / 100 + 60, this._svgWidth * (1 - this._padding / 100)];
    } else {
      return [this._svgWidth * this._padding / 100, this._svgWidth * (1 - this._padding / 100) - 60];
    }
  }
  ...
  set infoPanelType(val) {
    _classPrivateFieldSet(this, _infoPanelType, val);
  ...
  loadLineNum() {
    ...
    if (_classPrivateFieldGet(this, _lineNum).length === 2) {
  ...
  loadLineName() {
    ...
    if (this._lineNames[1].length > 10) {
      $('#line_name text:last-child').text(this._lineNames[1]).addClass('rmg-name__gzmtr--int-small').removeClass('rmg-name__gzmtr--int');
    } else {
      $('#line_name text:last-child').text(this._lineNames[1]).removeClass('rmg-name__gzmtr--int-small').addClass('rmg-name__gzmtr--int');
    }
  ...
  drawDestInfo() {
    ...
    $('#station_info_gzmtr #indicator_light').attr({
      ...
      y: 270,
      'xlink:href': '#indicator_' + _classPrivateFieldGet(this, _infoPanelType)
    });
...
var _infoPanelType = new WeakMap();