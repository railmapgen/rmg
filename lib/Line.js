...
class Line {
  constructor(param) {
    ...
    _classPrivateFieldSet(this, _charForm, param.char_form); // Calculate other properties of stations
  ...
  set charForm(val) {
    _classPrivateFieldSet(this, _charForm, val);
  ...
  _pathWeight(stnId1, stnId2) {
    ...
    if (this._stnOutdegree(stnId1) == 2) {
      w += _classPrivateFieldGet(this, _longInterval);
    }

    if (this._stnIndegree(stnId2) == 2) {
      w += _classPrivateFieldGet(this, _longInterval);
    }
  ...
  _stnNamePos(stnId) {
    ...
    if (pos == -1) {
      if (_classPrivateFieldGet(this, _stations)[stnId]._parents == 'linestart') {
        return 1;
      }

      return Number(!self._stnNamePos(_classPrivateFieldGet(this, _stations)[stnId]._parents[0]));
    }
  ...
  drawLine() {
    ...
    for (let [leftStnId, leftStnInstance] of Object.entries(_classPrivateFieldGet(this, _stations))) {
      ...
      for (let rightStnId of leftStnInstance._children) {
        ...
        if (y1 == y2) {
          $(`#${lineType}`).append($('<path/>').attr('d', `M ${x1},${y1} H ${x2}`));
        } else if (y1 == this.y && y1 > y2) {
          if (['Int3RStation', 'OSI11RStation', 'OSI12RStation'].includes(leftStnInstance.constructor.name)) {
            $(`#${lineType}`).append(`<path d="M ${x1},${y1} h ${extraH / 2 + dh} a ${r},${r} 0 0,0 ${dx},${-dy} a ${r},${r} 0 0,1 ${dx},${-dy} H ${x2}"/>`);
          } else {
            $(`#${lineType}`).append(`<path d="M ${x1},${y1} h ${extraH / 2 + dh} a ${r},${r} 0 0,0 ${dx},${-dy} a ${r},${r} 0 0,1 ${dx},${-dy} H ${x2}"/>`);
          }
        } else if (y1 == this.y && y1 < y2) {
          if (['Int3RStation', 'OSI11RStation', 'OSI12RStation'].includes(leftStnInstance.constructor.name)) {
            $(`#${lineType}`).append(`<path d="M ${x1},${y1} h ${extraH / 2 + dh} a ${r},${r} 0 0,1 ${dx},${dy} a ${r},${r} 0 0,0 ${dx},${dy} H ${x2}"/>`);
          } else {
            $(`#${lineType}`).append(`<path d="M ${x1},${y1} h ${extraH / 2 + dh} a ${r},${r} 0 0,1 ${dx},${dy} a ${r},${r} 0 0,0 ${dx},${dy} H ${x2}"/>`);
          }
          ...
          if (['Int3LStation', 'OSI11LStation', 'OSI12LStation'].includes(_classPrivateFieldGet(this, _stations)[rightStnId].constructor.name)) {
            $(`#${lineType}`).append(`<path d="M ${x2},${y2} h ${-extraH / 2 - dh} a ${r},${r} 0 0,1 ${-dx},${-dy} a ${r},${r} 0 0,0 ${-dx},${-dy} H ${x1}"/>`);
          } else {
            $(`#${lineType}`).append(`<path d="M ${x2},${y2} h ${-extraH / 2 - dh} a ${r},${r} 0 0,1 ${-dx},${-dy} a ${r},${r} 0 0,0 ${-dx},${-dy} H ${x1}"/>`);
          }
        } else {
          if (['Int3LStation', 'OSI11LStation', 'OSI12LStation'].includes(_classPrivateFieldGet(this, _stations)[rightStnId].constructor.name)) {
            $(`#${lineType}`).append(`<path d="M ${x2},${y2} h ${-extraH / 2 - dh} a ${r},${r} 0 0,0 ${-dx},${dy} a ${r},${r} 0 0,1 ${-dx},${dy} H ${x1}"/>`);
          } else {
            $(`#${lineType}`).append(`<path d="M ${x2},${y2} h ${-extraH / 2 - dh} a ${r},${r} 0 0,0 ${-dx},${dy} a ${r},${r} 0 0,1 ${-dx},${dy} H ${x1}"/>`);
          }
        }
  ...
  fillThemeColour() {
    ...
    // }
    $('#line_main, #strip, #dest_strip').attr('stroke', _classPrivateFieldGet(this, _themeColour));
  ...
  drawDestInfo() {
    ...
    $('#dest_name > g:last-child').empty().attr('text-anchor', txtAnchor).append($('<text>').addClass('NameZH DestName').text(`${lineNameZH}往${destNameZH}`)).append($('<text>', {
      'dy': 80,
      'class': 'NameEN DestName'
  ...
  loadFonts() {
    $('.NameZH').addClass(`char-${_classPrivateFieldGet(this, _charForm)}`);
  }
