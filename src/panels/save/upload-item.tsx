...
import { ListItem, ListItemIcon, Icon, ListItemText, Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions, Button } from '@material-ui/core';
    ...
    return (
        <div>
            ...
            <input type="file" accept="application/json" 
                style={{display: 'none'}} id="upload-param" onChange={handleUpload} />
...
function ImportDialog(props: ImportDialogProps) {
    ...
    const describeParams = (param: RMGParam) => {
        return `Number of stations: ${Object.keys(param.stn_list).length-2}
                ${Object.entries(param.stn_list).map(x => ['linestart','lineend'].includes(x[0]) ? '' : x[1].name.join(' - ')).join('\r\n').trim().replace(/\\/,' ')}`;
    }
