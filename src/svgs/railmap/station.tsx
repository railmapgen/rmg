...
const Station = (props: Props) => {
    ...
    const branchDy = React.useMemo(() => {
        ...
        if (affix === '') {
            ...
            let pos = branchPos;
            if (pos.includes('SE') || pos.includes('SW')) { return props.namePos ? 9.68 : 0; }
            if (pos.includes('NE') || pos.includes('NW')) { return props.namePos ? 0 : -9.68; }
        }
    ...
    const branchElDy = React.useMemo(() => {
        ...
        if (affix === '') {
            ...
            let pos = branchPos;
            if (pos.includes('SE') || pos.includes('SW')) { return props.namePos ? 0 : 9.68; }
            if (pos.includes('NE') || pos.includes('NW')) { return props.namePos ? -9.68 : 0; }
        }
    ...
    return (
        <>
            <g style={{ transform: `translateY(${branchElDy}px)` }}>
                <IntTickGroup
                    variant={
                        (stnInfo.transfer.type === 'osi22' &&
                            ...
                            'osi22end' : stnInfo.transfer.type}
                ...
                {stnInfo.transfer.type.includes('osi') &&
                    <OSIName
                        ...
                        stnState={props.stnState}
                        variant={(stnInfo.transfer.type === 'osi22' &&
                            (stnInfo.parents[0] === 'linestart' || stnInfo.children[0] === 'lineend')) ?
                            'osi22end' : stnInfo.transfer.type as any}
            ...
            <use
                ...
                className={(props.stnState === -1 ? 'rmg-stn__mtr--pass' : 'rmg-stn__mtr--future') +
                    (stnInfo.transfer.paid_area ? ' rmg-stn__mtr--paid-osi' : ' rmg-stn__mtr--unpaid-osi')}
                style={{
                    transform: `translateY(${branchDy}px)` +
            ...
            <g style={{ transform: `translateY(${branchDy}px)` }}>
                <StationNameGElement
                    ...
                    nameDX={(stnInfo.transfer.type === 'osi22' && stnIcon === 'osi12') ?
                        (stnInfo.transfer.tick_direc === 'l' ? 3 : -3) : null} />
            </g>
        </>
    );
};
...
const StationNameGElement = (props: StationNameGElementProps) => {
    ...
    const facilityX = !props.nameDX ? -(bBox.width + 3) / 2 :
        (props.nameDX > 0 ?
            props.nameDX + (NAME_FULL_HEIGHT + 2) / 2 :
            - (NAME_FULL_HEIGHT + 2) / 2 - bBox.width + props.nameDX);
    const facilityNameDX = props.facility === '' ? 0 :
        (!props.nameDX ?
            (NAME_FULL_HEIGHT + 2 + 3) / 2 :
            (props.nameDX < 0 ? 0 : NAME_FULL_HEIGHT + 2 + 3 + props.nameDX));
    ...
    return (
        <g textAnchor={textAnchor}
            ...
            {props.stnState === 0 && <rect id="current_bg"
...
const IntTickGroup = (props: IntTickGroupProps) => {
    switch (props.variant) {
            ...
            return (
                <>
                    {props.stnTrans.info[0].map((intInfo, i) => (
                        <g key={i}
                            style={{
                                transform: `translateY(${!props.namePos ? 18 * (i + 1) : -18 * (props.stnTrans.info[0].length - i)}px)`
                            }}>
                            <IntTick
                                intInfo={intInfo}
                                stnState={props.stnState}
        ...
        case 'osi11':
            return (
                <g style={{ transform: `translateY(${props.namePos ? -26 : 26}px)` }}>
                    <IntTick
            ...
            return (
                <>
                    {props.stnTrans.info[1].map((intInfo, i) => (
                        <g key={i} style={{
                            transform: `translateY(${!props.namePos ? 8 + 18 * (i + 1) : -8 - 18 * (props.stnTrans.info[1].length - i)}px)`
                        }}>
                            <IntTick
                                intInfo={intInfo}
                                stnState={props.stnState}
...
const IntTick = (props: IntTickProps) => {
    ...
    return React.useMemo(() => (
        <>
            <use xlinkHref="#inttick"
                stroke={props.intInfo[2]} style={{ transform: `rotate(${props.rotation}deg)` }}
                className={'rmg-line rmg-line__mtr rmg-line__change' +
                    (props.stnState === -1 ? ' rmg-line__pass' : '')} />
            <g textAnchor={textAnchor}
                style={{ transform: `translate(${x + (props.nameDX || 0)}px,${y}px)` }}
                className={`Name ${props.stnState === -1 ? 'Pass' : 'Future'}`}>
                {props.intInfo[4].split('\\').map((txt, i) => (
                    <text key={i} className="rmg-name__zh IntName" dy={10 * i}>{txt}</text>
                ))}
                {props.intInfo[5].split('\\').map((txt, i) => (
                    <text key={nameZHLns + i} className="rmg-name__en IntName" dy={nameZHLns * 10 - 1 + 7 * i}>{txt}</text>
                ))}
            </g>
        </>
...
const OSIName = (props: OSINameProps) => {
    ...
    const y = (variant => {
        switch (variant) {
            ...
            case 'osi22':
                return (!props.namePos ? (26 - 18) : -8) -
                    (props.namePos ? 18 + 9 : -27) +
                    8.34375 - 25.03125 / 2 - 5 * (props.name[1].split('\\').length - 1);
            case 'osi22end':
                return !props.namePos ?
                    (10) + 8.34375 :
    ...
    return (
        <g textAnchor={textAnchor}
            ...
            {props.name[1].split('\\').map((txt, i) => (
                <text key={i} className="rmg-name__en rmg-name__mtr--osi" dy={12 + 10 * i}>{txt}</text>
            ))}
        </g>
    );
};